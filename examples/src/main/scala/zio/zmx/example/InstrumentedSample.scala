...
trait InstrumentedSample {
  ...
  val aspHistogram =
    ZIOMetric.observeHistogram("zmxHistogram", ZIOMetric.Histogram.Boundaries.linear(0.0d, 10.0d, 11))
  ...
  private lazy val gaugeSomething = for {
    _ <- Random.nextDoubleBetween(0.0d, 100.0d) @@ aspGaugeAbs @@ aspCountAll
    _ <- Random.nextDoubleBetween(-50d, 50d) @@ aspGaugeRel @@ aspCountAll
  } yield ()
