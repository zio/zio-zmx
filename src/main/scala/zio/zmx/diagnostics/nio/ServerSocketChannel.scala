...
final class ServerSocketChannel(val channel: JServerSocketChannel) {

  final def bind(addr: InetSocketAddress): IO[IOException, Unit] =
  ...
  final val validOps: UIO[Integer] =
  ...
  final def accept: IO[IOException, SocketChannel] =
